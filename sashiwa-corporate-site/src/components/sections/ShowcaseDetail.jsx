  ...
  return (
    <div className="bg-white">
      ...
      <div className="mx-auto max-w-5xl px-6 sm:px-10 lg:px-14 pt-8 pb-4">
        {item.client && <p className="text-xs font-bold tracking-[0.2em] uppercase text-red-600">{item.client}</p>}
        ...
        {item.metric && (
          <div className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200 px-4 py-1.5">
            ...
            <span className="text-sm font-semibold text-emerald-700">{item.metric}</span>
          </div>
        )}
      </div>
      ...
      {item.image?.url && (
        <div className="mx-auto max-w-5xl px-6 sm:px-10 lg:px-14">
          <div className="relative aspect-[16/9] rounded-3xl overflow-hidden shadow-[0_30px_60px_-20px_rgba(0,0,0,0.25)]">
      ...
      <div className="mx-auto max-w-5xl px-6 sm:px-10 lg:px-14 py-16 sm:py-20">
        {item.body ? (
          ...
          <div className="prose prose-neutral max-w-none text-neutral-600 leading-loose" dangerouslySetInnerHTML={{ __html: item.body }} />
        ) : (
        ...
        <div className="mt-16 rounded-3xl border border-neutral-200 bg-neutral-50 p-8 sm:p-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <h3 className="text-xl font-black text-neutral-950">同じような課題を解決したい</h3>
            <p className="mt-2 text-sm text-neutral-500">貴社の状況に合わせて、担当AIエージェントがご提案いたします。</p>
          </div>
